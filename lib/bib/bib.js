...
class ZoteroBib {
	constructor(opts) {
		...
		if (this.opts.persist && this.opts.storage) {
			...
			this.items = [...this.opts.initialItems, ...this.getItemsStorage()].filter(isLikeZoteroItem);
	...
	async translateIdentifier(identifier, _ref = {}) {
		let { endpoint = '/search' } = _ref,
		    opts = _objectWithoutProperties(_ref, ['endpoint']);
	...
	async translateUrlItems(url, items, _ref2 = {}) {
		let { endpoint = '/web' } = _ref2,
		    opts = _objectWithoutProperties(_ref2, ['endpoint']);

		let translateURL = `${this.opts.translateURL}${this.opts.translatePrefix}${endpoint}`;
		let sessionid = this.opts.sessionid;
		let data = _extends({ url, items, sessionid }, this.opts.request);
	...
	async translateUrl(url, _ref3 = {}) {
		let { endpoint = '/web' } = _ref3,
		    opts = _objectWithoutProperties(_ref3, ['endpoint']);

		let translateURL = `${this.opts.translateURL}${this.opts.translatePrefix}${endpoint}`;
		let sessionid = this.opts.sessionid;
		let data = _extends({ url, sessionid }, this.opts.request);
